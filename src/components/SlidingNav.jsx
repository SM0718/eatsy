...
import HiddenCart from './HiddenCart'
...
function SlidingNav({className, setShow, headerItems}) {
  ...
  return (
    <div className={`${className} w-full md:w-[350px] h-screen bg-white shadow-xl z-50`}>
        <div className='w-full relative flex flex-col items-center justify-between py-8'>
          <div className='relative'>
            <Logo />
            <span onMouseEnter={() => setChange(true)} onMouseLeave={() => setChange(false)} onClick={() => setShow(false)} className='cursor-pointer absolute -top-8 -right-32'>
                <Cross change={change}/>
            </span>

            <HiddenCart className={'absolute'}/>
          </div>
          <ul className='w-2/3 flex flex-col gap-4'>
            {
              headerItems.map(items => <li onClick={() => setShow(false)} className='w-full text-center text-[16px] border-b-2 pb-4'>
